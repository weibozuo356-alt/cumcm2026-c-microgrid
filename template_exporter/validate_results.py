...
from export_results import FILES, emergency_events, read_solution
...
def validate_file(root: Path, name: str, solution: pd.DataFrame) -> tuple[dict[str, object], list[str]]:
    ...
    price_data = pd.read_csv(root / "processed_timeseries.csv", encoding="utf-8-sig", usecols=["date", "interval_index", "price_actual", "price_static"])
    price_data["date"] = pd.to_datetime(price_data["date"]).dt.normalize()
    cg = g.merge(price_data, on=["date", "interval_index"], how="left")
