...
def main() -> None:
    ...
    template_dir = root / "C题" / "附件" / "附件5"
    reports = [export_one(name, template_dir, root, solution) for name in FILES]
    print(json.dumps({"status": "ok", "solution": str(solution_path), "files": reports}, ensure_ascii=False))
